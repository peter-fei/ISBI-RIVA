...
classes = ('CELL',)  # 将 'defect' 替换为你实际的类别名
num_classes = 1

resume_from = None
load_from = '/public_bme2/bme-wangqian2/jianght/ckpts/pretrained/co-detr-vit-large-coco-instance.pth'
pretrained = None
dataset_type = 'CocoDataset'
data_root = '/public_bme2/bme-wangqian2/jianght/Datas/ISBI-riva-cervical-cytology-challenge-track-b/'
img_norm_cfg = dict(
...
train_pipeline = [
    ...
    dict(type='Resize', 
         img_scale=(1024, 1024), # 在 800 到 1280 之间随机采样
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        classes=classes, 
        ann_file=data_root + 'annotations/train_coco_keypoints_seg.json',
        img_prefix=data_root + 'images/train/',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        classes=classes, 
        ann_file=data_root + 'annotations/val_coco_keypoints_seg.json',
...
model = dict(
    ...
    mask_head=dict(
        ...
        stage_num_classes=[1, 1, 1, 1],  # use class-agnostic classifier in the last stage
    ...
    test_cfg=[
        dict(
            max_per_img=100,
            # nms=dict(type='soft_nms', iou_threshold=0.9),
            # mask_thr_binary=0.8
            ),
        dict(
            score_thr=0.0,
...
evaluation = dict(
    interval=1,            # 每 1 个 epoch 评估一次
    metric='bbox', 
    save_best='bbox_mAP',  # 自动保存 mAP 最高的模型
...
work_dir = '/public_bme2/bme-wangqian2/jianght/ckpts/ISBI_RIVA/B/co_detr_vit_instance2'
